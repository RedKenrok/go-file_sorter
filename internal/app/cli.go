...
var (
	flagHelp     = flag.Bool("help", false, "Show detailed help information")
	helpShort    = flag.Bool("h", false, "Show detailed help information")
	flagVersion  = flag.Bool("version", false, "Show program version information")
...
func RunCLI(
	...
	if *flagHelp || *helpShort {
		showHelp()
		os.Exit(0)
	}
	if *flagVersion || *versionShort {
		showVersion(
			version,
			commit,
			buildDate,
	...
	err := filepath.Walk(
		...
		func(
			...
			count++
			newFileName := file.FormatName(pathFormat, creationDate, count, path)
